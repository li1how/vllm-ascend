...
- KV cache pooling with PCP requires `AscendStoreConnector` with `use_layerwise=false`; layerwise transfer is not supported.
- Pool reuse across PCP sizes requires compatible models, cache layouts, block sizes, hash settings, and other parallel dimensions. Legacy PCP-sharded pool entries must not be reused.
