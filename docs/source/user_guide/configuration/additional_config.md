...
| `enable_pcp_o_proj_weight_sharding` | bool | `False` | Whether PCP shards O-projection weights across the PCP group. This single option controls both SFA-PCP `o_proj` and DSA-PCP two-stage O projection (`wo_a`/`wo_b`). |
| `enable_pcp_embedding_lmhead_weight_sharding` | bool | `False` | Whether DSA-PCP shards embedding and LM Head weights across PCP ranks inside each TP shard. Requires PCP > 1 and a DSA model with a Lightning Indexer, and is incompatible with `enable_reduce_sample` and fine-grained TP for these modules. |
