...
| `enable_pcp_decode_sharding`        | bool | `True`  | Whether MRV2 shards decode requests across PCP ranks when PCP > 1 and DCP = 1. Set to `false` to keep decode requests replicated at the same topology. |
