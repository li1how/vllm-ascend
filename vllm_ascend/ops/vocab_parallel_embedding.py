...
def _resolve_vocab_parallel_plan(
    *,
    ...
    use_pcp_sharding = enable_pcp_embedding_lmhead_weight_sharding()

    if use_pcp_sharding and is_token_embedding and embedding_tp_enable():
        raise ValueError(
            "PCP embedding weight sharding cannot be combined with "
            "finegrained_tp_config.embedding_tensor_parallel_size."
        )
    if use_pcp_sharding and is_lm_head and lmhead_tp_enable():
        raise ValueError(
            "PCP LM head weight sharding cannot be combined with finegrained_tp_config.lmhead_tensor_parallel_size."
        )

    if use_pcp_sharding and (is_token_embedding or is_lm_head):
